...
## Human interaction and accessibility refinement

First-use keyboard navigation must have a working bypass path. Conditional actions must leave focus on a stable, meaningful control; asynchronous completion must not move focus away from newer activity. Form errors must be associated with the relevant current input. Visually represented data must remain available as semantic text. Essential controls/help text have a 12px floor and practical minimum heights. At 320px and doubled computed text, the document must reflow; intentionally wide data keeps labelled keyboard-scrollable regions. Reduced motion and forced colors preserve focus/meaning. Acceptance and exact keyboard paths are recorded in [ACCESSIBILITY.md](docs/ACCESSIBILITY.md) and `tests/e2e/accessibility.spec.ts`; these do not assert screen-reader, physical-device or full WCAG conformance.
